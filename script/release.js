...
const run = (bin, args, opts = {}) =>
	!isTest ? execa(bin, args, { stdio: "inherit", ...opts }) : progress(`${bin} ${args.join(" ")}`)
...
const chooseVersion = async () => {
	...
	if (!targetVersion) {
		const { type } = await prompt({
			...
			choices: releaseTypes.map(type => `${type} (${incVersion(type)})`).concat(["custom"])
		})
		if (type === "custom") {
			targetVersion = (
				await prompt({
...
const checkCurrentBranch = async () => {
	...
	if (branch !== "main" && !isTest) {
		throw new Error("Release branch must be main, please checkout main branch and try it again!")
	}
}
